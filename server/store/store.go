...
func Migrate(db *sql.DB) error {
	...
	files := []string{"migrations/0001_init.sql", "migrations/0002_forwarding.sql", "migrations/0003_render_form.sql", "migrations/0004_preview_of.sql", "migrations/0005_guest_claim.sql", "migrations/0006_search.sql"} // append in order
	for i, f := range files {
		version := i + 1
		if version <= v {
			continue
