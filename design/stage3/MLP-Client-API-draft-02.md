# MLP Client API — draft-02 (informative companion)

> **draft-02 changes (S4.19, D-261).** One additive endpoint:
> `GET /search` — node-local full-text search over the mailbox's
> messages, including text extracted from held media. No existing
> endpoint changed. draft-01 clients are unaffected.
...
| Method & path | Purpose |
|---|---|
| GET `/search?q=&limit=&offset=` | Full-text search, newest first: subjects, body derived text (§11.6), media names, and text extracted from held media (documents are just a media type). Results are grouped per message with per-door matches (`via: message\|media`) and bracket-marked snippets. The index is node-local derived data (FTS over `medialets.derived_text` + `object_text`), rebuildable at will; **search never crosses the wire** — a cross-domain search surface would cut against envelope privacy (D-04). |
...
*Supersedes draft-01. Companion to: MLP Flagship Client Design S3.9. Core-spec relation:
