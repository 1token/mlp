...
**D-279** search interaction: nav-resident input; keystrokes touch
only the `search` slice; empty↔active transitions alone re-render the
shell (focus preserved, explicit refocus after the swap); 250 ms
debounce, Enter immediate, Escape clears; stale responses dropped by
live-query comparison. · **D-280** `<mlp-search>` is a pure renderer;
the shell owns fetching (the tab-fetch pattern); keyed reconcile;
thread navigation via the same `nav.openThread` path; results survive
the round trip. · **D-281** snippet safety: `snippetHtml` in
`lib/html.js` under the escaping gate — escape everything, then
convert D-267 bracket pairs to `<mark>`; the one greppable
`unsafe()`; literal-bracket ambiguity accepted as cosmetic; run-html
